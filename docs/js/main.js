...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var projectData = {
    ...
    '계근 시스템': {
      problem: '화물 차량 중량 측정 결과를 수기로 기록·관리하여 데이터 오류와 이력 추적 어려움 발생.',
      role: '2인 팀 중 백엔드 담당 — 데이터 입력 API, DB 설계, JSP 화면 퍼블리싱.',
      solution: '실시간 중량 입력 화면 및 이력 조회 API 개발. MySQL 테이블 설계로 데이터 무결성 확보. 엑셀 내보내기 기능 추가.',
      result: '수기 관리에서 시스템화 전환 완료. 이력 조회·출력 기능으로 업무 효율 향상.',
      tech: ['Java', 'Spring', 'JSP', 'MySQL']
    },
    '5G/PC 반입반출 시스템': {
      problem: '보안 구역 내 5G 기기·PC 반입반출을 구두 및 수기로 관리하여 보안 감사 이슈 발생.',
      role: '2인 팀 중 백엔드 담당 — 승인 워크플로우 설계 및 API 개발, 이력 추적 기능.',
      solution: '신청→승인→반려→이력보관 워크플로우 설계 및 구현. Oracle DB 이력 테이블 설계. 관리자 승인 대시보드 개발.',
      result: '보안 감사 기준 충족. 반입반출 현황 실시간 조회 가능. 이력 보존으로 사후 감사 대비.',
